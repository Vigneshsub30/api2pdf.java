...
class Post_Chrome_HtmlMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Chrome_HtmlHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("options")) {
            queryParams.add("options=" + args.get("options"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_chrome_html" + queryString;
    ...
    public static MCPServer.Tool createPost_Chrome_HtmlTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Chrome_HtmlHandler(config));
    }
