...
class Get_Chrome_UrlMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_Chrome_UrlHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("output")) {
            queryParams.add("output=" + args.get("output"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_chrome_url" + queryString;
    ...
    public static MCPServer.Tool createGet_Chrome_UrlTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getGet_Chrome_UrlHandler(config));
    }
