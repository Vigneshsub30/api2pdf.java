/**
 * MCP Server function for Convert office document or image to PDF
 */
...
class Post_Libreoffice_ConvertMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Libreoffice_ConvertHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("inlinePdf")) {
            queryParams.add("inlinePdf=" + args.get("inlinePdf"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_libreoffice_convert" + queryString;
    ...
    public static MCPServer.Tool createPost_Libreoffice_ConvertTool(MCPServer.APIConfig config) {
        ...
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_libreoffice_convert",
            "Convert office document or image to PDF",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_Libreoffice_ConvertHandler(config));
    }
